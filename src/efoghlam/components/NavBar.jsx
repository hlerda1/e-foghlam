          ...
          <Container fluid>
            <Navbar.Brand>
              <NavLink className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} to='/auth/login'>
                <span>
                  <img src="Foghlan_logo_trans.png" alt="Logo" width={40} height={40}/>
                    &nbsp;
                  {user.nombre}
                </span>
              </NavLink>
            ...
            >
              ...
              <Offcanvas.Body>
                <Nav className="justify-content-end flex-grow-1 pe-3">
                  {show ? (<NavLink className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} to='/comisiones'>Comisiones</NavLink>) : null}
                  <NavLink className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} to='/calendario'>Calendario</NavLink>
                  {show ? (<NavLink className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} to='/actividades'>Actividades</NavLink>) : null}
                  <NavLink className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} to='/notas'>Notas</NavLink>
                  <NavLink className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`} to='/comunicaciones'>Comunicaciones</NavLink>
                  <button onClick={startLogout} className='btn btn-outline-danger'>
                       <i className='fas fa-sign-out alt'></i>
